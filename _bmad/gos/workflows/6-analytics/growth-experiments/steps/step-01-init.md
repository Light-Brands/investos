---
name: 'step-01-init'
description: 'Initialize the growth experiments workflow by detecting continuation state and setting up the document'
nextStepFile: './step-02-hypothesis-generation.md'
outputFile: '{growth_artifacts}/growth-experiments-{{project_name}}-{{date}}.md'
templateFile: '../growth-experiments.template.md'
---

# Step 1: Growth Experiments Initialization

## STEP GOAL:
Initialize the growth experiments workflow by detecting continuation state, loading relevant context documents, and setting up the document structure.
...
- You bring experimental design and statistical methodology expertise; the user brings business context and product knowledge

### Step-Specific Rules:
- Focus only on initialization and setup - no experiment content yet
...
"Welcome {user_name}! I've set up your growth experiments workspace for {project_name}.
...
- Campaign performance: {count or 'None found'}
...
ONLY WHEN setup completion is achieved and frontmatter properly updated, will you then read fully and follow: `{nextStepFile}` to begin hypothesis generation.
