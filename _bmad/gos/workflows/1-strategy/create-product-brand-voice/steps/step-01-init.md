---
name: 'step-01-init'
description: 'Initialize the product brand voice workflow by detecting continuation state and setting up the document'

nextStepFile: './step-02-brand-discovery.md'
outputFile: '{growth_artifacts}/product-brand-voice-{{project_name}}-{{date}}.md'
templateFile: '../product-brand-voice.template.md'
---

# Step 1: Brand Voice Initialization

## STEP GOAL:
Initialize the brand voice workflow by detecting continuation state, loading relevant context documents, and setting up the document structure.
...
- You bring creative strategy; the user brings product vision
...
- Existing brand guidelines (`*brand*.md`, `*voice*.md`, `*style*.md`)
...
"Welcome {user_name}! I've set up your brand voice workspace for {project_name}.
...
- Existing brand work: {count or 'None found'}

Do you have any other documents, brand references, or inspiration you'd like me to include, or shall we continue?"

### 4. Present MENU OPTIONS
After setup report, read fully and follow: {nextStepFile}

## CRITICAL STEP COMPLETION NOTE
ONLY WHEN setup completion is achieved and frontmatter properly updated, will you then read fully and follow: `{nextStepFile}` to begin brand discovery.
