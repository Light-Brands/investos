---
name: 'step-01-init'
description: 'Initialize the email sequences workflow by detecting continuation state and setting up the document'

nextStepFile: './step-02-audience-segmentation.md'
outputFile: '{growth_artifacts}/email-sequences-{{project_name}}-{{date}}.md'
templateFile: '../email-sequences.template.md'
---

# Step 1: Email Sequences Initialization

## STEP GOAL:
Initialize the email sequences workflow by detecting continuation state, loading relevant context documents, and setting up the document structure.
...
- You bring email marketing methodology; the user brings audience knowledge

### Step-Specific Rules:
- Focus only on initialization and setup - no sequence design yet
...
- Existing email work (`*email*.md`, `*sequence*.md`, `*nurture*.md`)
...
"Welcome {user_name}! I've set up your email sequences workspace for {project_name}.
...
- Existing email work: {count or 'None found'}

Do you have any other documents or email data you'd like me to include, or shall we continue?"

### 4. Present MENU OPTIONS
After setup report, read fully and follow: {nextStepFile}

## CRITICAL STEP COMPLETION NOTE
ONLY WHEN setup completion is achieved and frontmatter properly updated, will you then read fully and follow: `{nextStepFile}` to begin audience segmentation.
